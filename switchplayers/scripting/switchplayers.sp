...
void PerformSwitch(int client, int target, int team, bool silent)
{
	...
	if( h_Switch_CheckTeams.BoolValue )
	{
	...
	if( team == 2 )
	{
		...
		int bot = 1;
		while ( !(IsClientInGame(bot) && IsFakeClient(bot) && GetClientTeam(bot) == 2) )
		{
			bot++;
		}

...
{
	CreateConVar("l4d_votemanager_version", PLUGIN_VERSION, "Version of Vote Manager", CVAR_FLAGS|FCVAR_DONTRECORD);

	hCooldownMode = CreateConVar("l4d_votemanager_cooldown_mode", "0", "0 = cooldown is shared 1 = cooldown is independant", CVAR_FLAGS, true, 0.0, true, 1.0);
	hVoteCooldown = CreateConVar("l4d_votemanager_cooldown", "60.0", "Clients can call votes after this many seconds", CVAR_FLAGS, true, 0.0, true, 300.0);
	hTankImmunity = CreateConVar("l4d_votemanager_tank_immunity", "1", "Tanks have immunity against kick votes", CVAR_FLAGS, true, 0.0, true, 1.0);
	hRespectImmunity = CreateConVar("l4d_votemanager_respect_immunity", "1", "Respect admin immunity levels in kick votes (only when admin kicking admin)", CVAR_FLAGS, true, 0.0, true, 1.0);
	hLog = CreateConVar("l4d_votemanager_log", "3", "1 = Log vote info to files 2 = Log vote info to server; add the values together if you want", CVAR_FLAGS, true, 0.0, true, 3.0);
   	if (bLeft4Dead2)
	{
...
Action CustomVote(int client, int args)
{
	if (client == 0 || GetServerClientCount(true) == 0)
	{
...
{
	if (client == 0 || GetServerClientCount(true) == 0 || IsFakeClient(client))
	{	
...
*/
Action VotePass_2(UserMsg msg_id, BfRead bf, const int[] players, int playersNum, bool reliable, bool init)
{
...
void VotePass(Event event, const char[] name, bool dontBroadcast)
{
	LogVoteManager("%T", "Vote Passed", LANG_SERVER);
	VoteLogAction(-1, -1, "callvote (verdict 'passed')");
	ClearVoteStrings();
...
byte    team
*/
Action VoteFail_2(UserMsg msg_id, BfRead bf, const int[] players, int playersNum, bool reliable, bool init)
{
...
void VoteFail(Event event, const char[] name, bool dontBroadcast)
{
	LogVoteManager("%T", "Vote Failed", LANG_SERVER);
	VoteLogAction(-1, -1, "callvote (verdict 'failed')");
	ClearVoteStrings();
 ...
 */
bool ClientHasAccess(int client, const char[] what)
{
...
{
	if (client == 0|| strlen(userid) < 1) // empty userid/console can't call votes
	{
		ClearVoteStrings();
		return Plugin_Handled;
	}
	...
	if (bRespectImmunity && IsPlayerAdmin(target) && id != INVALID_ADMIN_ID && targetid != INVALID_ADMIN_ID) // both targets need to be admin.
	{
    ...
    switch (iCooldownMode)
    {
        case 0:
        {
            for (int i = 1; i <= MaxClients; i++)
            {
                if (IsClientInGame(i))
                {
                    iNextVote[i] = time + fVoteCooldown;
                }
            }
            return;
        }
        case 1:
        {
            iNextVote[client] = time + fVoteCooldown;
            return;
        }
    }
}
